...
void print_licensing(void) {
    ...
    printf("Licensed under the GNU Library General Public License (LGPL), "
           "version 3.0\n");
    printf(
    ...
    printf("This software incorporates by linkage code from the libintl and "
           "libiconv\n");
    printf("libraries, which are covered by the Library GNU Public License, "
           "available\n");
...
s32 main(s32 argc, const char** argv) {
    ...
    if (gpuID >= devCount) {
        printf("Error: Specified invalid GPU index (%d); %d CUDA devices "
               "present, numbered from zero.\n",
               gpuID, devCount);
        printf("\nValid CUDA devices:\n");
        for (s32 i = 0; i < devCount; ++i) {
    ...
    if ((deviceProps.major == 9999) && (deviceProps.minor == 9999)) {
        printf("Error: No CUDA hardware detected. CUDA device emulator is not "
               "supported.\n");
        exit(2);
    }
    ...
    if (cudaSetDevice(gpuID) == cudaErrorInvalidDevice) {
        printf("Error: Got Invalid Device Error setting %d as active CUDA "
               "device.\n",
               gpuID);
        exit(2);
    }
    ...
    if (!tester.allocate(megsToTest)) {
        printf("Error: unable to allocate %u MiB of GPU memory to test, "
               "bailing!\n",
               megsToTest);
        printf("Error text: %s\n", cudaGetErrorString(cudaGetLastError()));
        exit(2);
    } else {
        printf("Running %u iterations of tests over %u MB of GPU memory on "
               "card %d: %s\n\n",
    ...
    printf("Running memory bandwidth test over %u iterations of %u MB "
           "transfers...\n",
    ...
    for (u32 i = 0; i < maxIters; ++i) {
        printf("Test iteration %u (GPU %d, %d MiB): %u errors so far\n", i + 1,
               gpuID, tester.size(), accumulatedErrors);
        u32 errorCount;
    ...
    printf("Final error count after %u iterations over %u MiB of GPU memory: "
           "%u errors\n",
